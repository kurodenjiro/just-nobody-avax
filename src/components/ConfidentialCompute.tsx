...
export const ConfidentialCompute: React.FC<ConfidentialComputeProps> = ({ visible, onClose }) => {
    ...
    return (
        <motion.div
            ...
            exit={{ opacity: 0 }}
        >
            <div className="w-[800px] pixel-corners border border-slate-200 bg-nobody-charcoal shadow-card-lg overflow-hidden flex flex-col relative h-[500px]">
                ...
                <div className="flex-1 flex relative">
                    ...
                    {/* Agent A Zone */}
                    <div className="w-1/2 p-6 border-r border-slate-100 flex flex-col items-start bg-nobody-primary-soft/20">
                        <div className="flex items-center gap-3 mb-8">
                            <div className="w-10 h-10 rounded-full bg-nobody-charcoal border border-nobody-primary flex items-center justify-center text-nobody-primary shadow-card">
                                <CabalFigure size={22} />
                            </div>
                            <div>
                                <div className="text-nobody-primary font-semibold text-sm">Agent A (Shark)</div>
                        ...
                        <div className="space-y-4 w-full">
                            {lines.filter(l => l.side === 'A').map((l, i) => (
                                <motion.div key={i} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
                                    <div className="bg-nobody-charcoal border border-nobody-primary/30 shadow-card p-3 pixel-corners-sm text-xs text-slate-600">
                                        "{l.text}"
                                    </div>
                                </motion.div>
                            ))}
                    ...
                    {/* Agent B Zone */}
                    <div className="w-1/2 p-6 flex flex-col items-end bg-nobody-gold-soft/20">
                        <div className="flex items-center gap-3 mb-8 flex-row-reverse text-right">
                            <div className="w-10 h-10 rounded-full bg-nobody-charcoal border border-nobody-gold flex items-center justify-center text-nobody-gold shadow-card">
                                <CabalFigure size={22} />
                            </div>
                            <div>
                                <div className="text-nobody-gold font-semibold text-sm">Agent B (Seller)</div>
                        ...
                        <div className="space-y-4 w-full flex flex-col items-end">
                            {lines.filter(l => l.side === 'B').map((l, i) => (
                                <motion.div key={i} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
                                    <div className="bg-nobody-charcoal border border-nobody-gold/30 shadow-card p-3 pixel-corners-sm text-xs text-slate-600 text-right">
                                        "{l.text}"
                                    </div>
                                </motion.div>
                            ))}
                    ...
                    {lines.some(l => l.side === 'CENTER') && (
                        <motion.div
                            ...
                            className="absolute bottom-10 left-1/2 -translate-x-1/2 bg-nobody-gold text-nobody-ink px-6 py-3 font-semibold text-sm pixel-corners-sm shadow-card-lg w-max"
                        >
