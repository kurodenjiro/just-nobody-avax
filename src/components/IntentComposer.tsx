...
export const IntentComposer: React.FC<IntentComposerProps> = ({ intent, setIntent, onSubmit, isProcessing }) => {
    return (
        <div className="absolute top-[65%] left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] z-20 flex flex-col items-center gap-6">
            <div className={`w-full relative bg-white border transition-all duration-300 shadow-card-lg ${isProcessing ? 'border-nobody-mint' : 'border-slate-200'} rounded-2xl p-1.5`}>

                {/* Processing Spinner */}
                {isProcessing && (
                    <div className="absolute -top-3 -right-3 w-6 h-6 bg-white border border-nobody-mint rounded-full flex items-center justify-center z-10 animate-spin shadow-card">
                ...
                <input
                    ...
                    className="w-full bg-slate-50 text-slate-900 p-4 outline-none text-sm rounded-xl placeholder-slate-400 focus:bg-white transition-colors pr-24"
                ...
                <button
                    ...
                    className="absolute right-4 top-1/2 -translate-y-1/2 text-xs font-medium text-slate-500 hover:text-slate-900 border border-slate-200 hover:border-slate-300 bg-white px-2.5 py-1 rounded-lg transition-all"
                >
                    Enter
                </button>
