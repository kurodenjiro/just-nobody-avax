...
export const NodeConfig: React.FC<NodeConfigProps> = ({ visible, onClose }) => {
    ...
    return (
        ...
        >
            <div className="w-80 bg-nobody-charcoal border border-slate-200 pixel-corners shadow-card-lg text-xs overflow-hidden">
                {/* Tabs */}
                ...
                <div className="p-4">
                    {tab === "node" ? (
                        <>
                            ...
                            <div className="bg-slate-50 pixel-corners-sm p-3 border border-slate-100 text-center">
                                <div className="text-slate-400 mb-1 text-[11px] font-medium">Projected Rewards</div>
                                <div className="text-xl text-nobody-primary font-bold">0.0042 AVAX/kb</div>
                            </div>
                        </>
                    ) : (
                        <>
                            <div className="mb-4 bg-nobody-gold-soft p-2 pixel-corners-sm text-center">
                                <span className="text-nobody-gold font-semibold">Mode: Aggressive</span>
                            </div>
                            <div className="space-y-4 text-slate-500">
                                ...
                                <div className="text-[11px] bg-amber-50 text-amber-600 p-2 pixel-corners-sm border border-amber-200">
                                ...
                                <div className="mt-4 pt-4 border-t border-slate-100 flex justify-between items-center text-[11px]">
                                    ...
                                    <div className="h-1 flex-1 mx-2 bg-slate-100 rounded-full relative">
                                        <div className="absolute right-0 top-1/2 -translate-y-1/2 w-3 h-3 bg-nobody-gold rounded-full shadow-card" />
                                    </div>
                ...
                <div className="p-3 border-t border-slate-200 bg-slate-50 text-center">
                    <button className="text-slate-500 hover:text-nobody-primary transition-colors text-[11px] font-semibold">
...
const Toggle = ({ label, defaultChecked }: { label: string, defaultChecked?: boolean }) => (
    <div className="flex justify-between items-center">
        ...
        <input type="checkbox" defaultChecked={defaultChecked} className="accent-nobody-primary" />
    </div>
);
